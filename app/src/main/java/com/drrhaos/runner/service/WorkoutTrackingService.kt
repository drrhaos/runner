...
class WorkoutTrackingService : Service() {
    ...
    private fun updateLocation(location: Location) {
        ...
        if (isCurrentlyTracking && !currentSession.isPaused) {
            ...
            currentSession = currentSession.copy(
                ...
                avgPace = avgPace,
                calories = calories,
                gpsStatus = com.drrhaos.runner.ui.tracking.GpsStatus.FOUND
            )
    ...
    private fun startLocationUpdates() {
        ...
        if (!hasLocationPermission()) {
            ...
            currentSession = currentSession.copy(
                gpsStatus = com.drrhaos.runner.ui.tracking.GpsStatus.DENIED
            )
            sessionUpdateCallback?.invoke(currentSession)
            return
        }
        ...
        try {
            ...
            currentSession = currentSession.copy(
                gpsStatus = com.drrhaos.runner.ui.tracking.GpsStatus.LOST
            )
            sessionUpdateCallback?.invoke(currentSession)
        }
    }
