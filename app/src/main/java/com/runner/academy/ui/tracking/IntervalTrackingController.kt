...
/**
 * Owns IntervalEngine lifecycle, interval panel UI, and interval voice cues.
 */
class IntervalTrackingController(
    ...
    fun update(session: WorkoutSession, announce: Boolean = true) {
        ...
        if (announce && isVoiceEnabled()) {
            ...
            if (next != null && engine.consumeUpcomingWarning(state, remainingMs)) {
                voiceFeedback()?.announceIntervalUpcoming(
                    title = next.localizedTitle(context),
    ...
    private fun formatSegmentVoicePace(segment: WorkoutTemplateSegment): String? {
        ...
        return context.getString(
            ...
            FormatUtils.formatPaceForTTS(pace, context)
        )
    }
}
